...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (storedOrder) {
    document.getElementById(
      "plant-image"
    ).src = `./Assets/plant-${storedOrder.plant}.png`;
    document.getElementById(
      "pot-image"
    ).src = `./Assets/pots/${storedOrder.pot}.png`;
    document.getElementById(
      "soil-image"
    ).src = `./Assets/soil-${storedOrder.soil}.png`;
    document.getElementById("extras-image").src = storedOrder.extras
      ? `./Assets/${storedOrder.extras}.png`
      : "";

    document.getElementById("plant-name").textContent = capitalize(
      storedOrder.plant
    );
    document.getElementById("soil-type").textContent = getSoilName(
      storedOrder.soil
    );
    document.getElementById("pot-description").textContent = getPotDescription(
      storedOrder.pot
    );
    document.getElementById("pot-color").textContent = storedOrder.pot
      .split("-")
      .slice(-1)[0];
  ...
  function calculateTotalPrice(selection) {
    ...
    console.log(
      `Plant: ${selection.plant} - Price: ${prices.plants[selection.plant]}`
    );
    ...
    console.log(
      `Soil: ${selection.soil} - Price: ${prices.soils[selection.soil]}`
    );
    total += prices.soils[selection.soil];

    if (selection.extras) {
      console.log(
        `Extra: ${selection.extras} - Price: ${prices.extras[selection.extras]}`
      );
  ...
  function displayPriceBreakdown(selection) {
    ...
    breakdownHtml += `<li>Plant (${prices.plants[selection.plant]}): ${
      selection.plant
    }</li>`;
    ...
    breakdownHtml += `<li>Soil (${prices.soils[selection.soil]}): ${
      selection.soil
    }</li>`;

    if (selection.extras) {
      breakdownHtml += `<li>Extras (${prices.extras[selection.extras]}): ${
        selection.extras
      }</li>`;
    }
  ...
  async function checkInventory(selection) {
    ...
    const inventoryElement = document.querySelector(
      "#accordions-container details:nth-child(2) p"
    );
    let alerts = "";
    ...
    if (plantStock === 0) {
      alerts += `${capitalizeFirstLetter(
        selection.plant
      )} is out of stock. Please select a different plant.<br>`;
      outOfStock = true;
    } else if (plantStock < 10) {
      alerts += `${capitalizeFirstLetter(
        selection.plant
      )}: Only ${plantStock} items left in stock!<br>`;
      limitedStock = true;
    }

    const potStock = await getInventory("pot", potItemId);
    if (potStock === 0) {
      alerts += `${convertToReadableFormat(
        selection.pot
      )} is out of stock. Please select a different pot.<br>`;
      outOfStock = true;
    } else if (potStock < 10) {
      alerts += `${convertToReadableFormat(
        selection.pot
      )}: Only ${potStock} items left in stock!<br>`;
      limitedStock = true;
    }
    ...
    if (soilStock === 0) {
      alerts += `${capitalizeFirstLetter(
        selection.soil
      )} soil is out of stock. Please select a different soil.<br>`;
      outOfStock = true;
    } else if (soilStock < 10) {
      alerts += `${capitalizeFirstLetter(
        selection.soil
      )} soil: Only ${soilStock} items left in stock!<br>`;
      limitedStock = true;
    }
  ...
  async function getPlantInfo(plantId) {
    const apiUrl =
      "https://qfble0gquj.execute-api.us-east-2.amazonaws.com/plant-store/info";
  ...
  async function displayPlantInfo(plantId) {
    ...
    const descriptionElement = document.querySelector(
      "#accordions-container details:nth-child(3) p"
    );
...
document
  .getElementById("back-to-customization")
